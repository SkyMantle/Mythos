...
Три режими роботи, які перемикає одна робоча нитка:
  ...
  SWEEP   — суцільний прохід 400 МГц … 6 ГГц кроками з перекриттям
            під ширину каналу. На кожному кроці — спектр, пошук
            зайнятих ділянок, груба відсіювання за шириною смуги
            (2.5…35 МГц).
  INSPECT — окреме захоплення на inspect_ms (типово 25 мс) і перевірка
            рядкової частоти. Це відсіює Wi-Fi, LTE та завади.
            Свіповий буфер для лінії 15.7 кГц закороткий.
  LOCK    — утримання каналу: безперервне читання IQ у кільцевий буфер,
            знімки для декодування, цифрова AFC, віддача кадрів у веб.
