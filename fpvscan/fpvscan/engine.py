...
Три режими роботи, які перемикає одна робоча нитка:
  ...
  SWEEP   — суцільний прохід 400 МГц … 6 ГГц кроками зі смуги
            дискретизації. На кожному кроці — спектр, пошук зайнятих
            ділянок, груба відсіювання за шириною смуги.
  INSPECT — окреме захоплення ~25 мс; кандидат демодулюється і
            перевіряється на рядкову частоту. Це відсіює Wi-Fi, LTE
            та завади. Свіповий буфер для цього закороткий.
  LOCK    — утримання каналу: безперервне читання IQ у кільцевий
            буфер, цифрова AFC, декодування полів, WebP у веб.
